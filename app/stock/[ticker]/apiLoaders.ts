...
export async function companyNews(symbol: string) {
  const { date_string_1, date_string_2 } = giveDateString(1);
  const  news_request = await fetch(
...
export function giveDateString(n: number) {
  ...
  if (curr_date.getDay() > 0 && curr_date.getDay() < 6) {
    ...
    if (curr_date.getDay() == 0) {
      const newDateStamp = curr_stamp - 2 * 86400 * 1000;
      const dateToUse = new Date(newDateStamp);
      return {
      ...
      const satDateStamp = curr_stamp - 1 * 86400 * 1000;
      const satDateToUse = new Date(satDateStamp);
      return {
...
export async function fetchChartData24H(symbol: string) {
  ...
  const { date_string_1, date_string_2 } = giveDateString(n);
...
export async function fetchDataLongTerm(symbol: string, timeframe: number) {
  const { date_string_1, date_string_2 } = longTermDate(timeframe);
